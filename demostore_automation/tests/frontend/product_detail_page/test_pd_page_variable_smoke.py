...
class TestProductDetailPageVariableProduct:
    ...
    def setup(self, request):
        """
        Setup fixture to initialize product data and navigate to the product page.
        """
        ...
        product_page = ProductPage(self.driver)  # Object created, constructor (__init__) gets executed
    ...
    def test_variable_product_page_verify_product_name(self):
        """
        Test method to verify that the product name displayed on the page matches the expected name.
        """
        logger.info('Running test: test_variable_product_page_verify_product_name')
        ...
        assert self.expected_name == displayed_name, f"Unexpected name displayed for a variable product. Expected: {self.expected_name}, Displayed: {displayed_name}"

    @pytest.mark.des23
    def test_variable_product_page_verify_main_image(self, setup):
        """
        Test method to verify that the main image URL displayed on the product page is correct.
        """
        ...
        assert displayed_img_url.endswith('jpg'), "The URL did not end with jpg"
        assert displayed_img_url.endswith(self.expected_url), f"Expected and displayed URLs do not match. " \
                                                              f"Expected: {self.expected_url}, Displayed: {displayed_img_url}"

    @pytest.mark.des24
    def test_variable_product_page_verify_extra_images(self, setup):
        """
        Test method to verify that the extra images displayed on the product page match the expected images.
        """
        ...
        # Compare the sorted lists of URLs
        assert sorted(all_displayed_urls) == sorted(self.all_expected_urls), "The expected and displayed alternate image URLs do not match."

    # Verify product type text on the product page
    @pytest.mark.des25
    def test_variable_product_page_verify_product_type_text(self, setup):
        """
        Test method to verify the product type text on the product page.
        """
    ...
    # Verify product price range before option selection
    @pytest.mark.des26
